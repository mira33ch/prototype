    ...
    stage('SonarQube Analysis Backend') {
        ...
        stage('Cleanup Artifacts') {
          ...
          stage('Trigger CD Pipeline') {
            steps {
                script {
                    sh "curl -v -k --user adminapplis:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'localhost:8080/job/gitops-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
        }
    }
